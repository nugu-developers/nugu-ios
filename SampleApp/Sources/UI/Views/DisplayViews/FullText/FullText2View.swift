...
final class FullText2View: DisplayView {
    ...
    override var displayPayload: Data? {
        didSet {
            guard let payloadData = displayPayload,
                let displayItem = try? JSONDecoder().decode(FullText2Template.self, from: payloadData) else { return }
    ...
    override func loadFromXib() {
        ...
        let view = Bundle.main.loadNibNamed("FullText2View", owner: self)?.first as! UIView
