...
final class FullText1View: DisplayView {
    ...
    override var displayPayload: Data? {
        didSet {
            guard let payloadData = displayPayload,
                let displayItem = try? JSONDecoder().decode(FullText1Template.self, from: payloadData) else { return }
    ...
    override func loadFromXib() {
        ...
        let view = Bundle.main.loadNibNamed("FullText1View", owner: self)?.first as! UIView
