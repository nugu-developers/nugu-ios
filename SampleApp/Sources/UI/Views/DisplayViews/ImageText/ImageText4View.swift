...
final class ImageText4View: DisplayView {
    ...
    override var displayPayload: Data? {
        didSet {
            guard let payloadData = displayPayload,
                let displayItem = try? JSONDecoder().decode(ImageText4Template.self, from: payloadData) else { return }
    ...
    override func loadFromXib() {
        ...
        let view = Bundle.main.loadNibNamed("ImageText4View", owner: self)?.first as! UIView
